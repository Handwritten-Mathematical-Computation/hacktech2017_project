...
# this server.py is modified from synaptics touchscreen demo folder.
    ...
    def getAllImages(self):
        ...
        while True:
            ...
            if p[0] == 2: # image packet
                ...
                if not has_Source:      #If there is a source
                    source_pixels = pixels #Get the base pixels. Pixels is the 2D array
                    count += 1
...
#                        exit()
                ...
                img = { 'timeStamp' : p[5] + (p[6] << 16),
                        ...
                        'image' : pixels }
                            ...
                            #print ("-------------------------")
#                small_pix = [[0 for x in range(72)] for y in range(46)]

                has_finger = False
                for i in range(46):
                    for j in range(72):
#                        print (has_Source)
                        difference = abs(source_pixels[i][j]-pixels[i][j]) #Difference of ambient noise?
                        # print("Difference : " , difference)
                        if difference > 30 and has_Source == True: #If difference is > 10
                            finger_Index.add((i,j)) #Put the touched locations into the finger_Index SET
                            pixels[i][j]
                            #print ("Index:",i,j) #Only print index value
                            has_finger = True
...
#                clear()
                ...
                if not has_finger: # that means no finger in the board, start the timer
                    # print("time passed since no finger : ", time.clock()-start_no_finger)
                    if (time.clock()-start_no_finger) > 3.00:
                        if len(finger_Index) > 15:
#                            print ("enough data to send to google!")
#                             with open("drawthis.txt",'w') as output_file:
#                                 for i in range(46):
#                                     for j in range(72):
#                                         if (i,j) in finger_Index:
#                                             output_file.write("x")
#                                         else:
#                                             output_file.write("-")
#                                     output_file.write("\n")
                            ...
                            time.sleep(3)
                            screen.fill(WHITE)
                            print("Draw final moment!")

                            if len(finger_Index) > 0:
                                for data in finger_Index:
                                    pygame.draw.circle(screen, BLUE, [(data[1] * 3 )+80, (data[0] * 3) +50], 3)
                            pygame.display.flip()
                            pygame.image.save(screen, 'real_life.png')
                            # exit()
                            time.sleep(1)
                            print("done!")
                    ...
                    if (time.clock()-start_no_finger) > 10.00:
                        finger_Index = set()
                else:
                    start_no_finger = time.clock()
                    # maybe here:
                    ...
                    if len(finger_Index) > 0:
                        for data in finger_Index:
                            pygame.draw.circle(screen, BLACK, [(data[1] * 5) +80, (data[0] * 5) +50], 2)
                    pygame.display.flip()

                # images.append(img) #List which contains all images
...
#                print("Discarding buffer...")
        ...
        #
        self.buffer = []
        # print ("Length of buffer FTDI: ", len(self.buffer))
...
if __name__ == '__main__':
    pygame.init()
    pygame.display.set_caption("Handwriting Mathmetical Calculation")
